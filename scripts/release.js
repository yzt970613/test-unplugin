...
try {
  ...
  // https://github.com/sindresorhus/execa/pull/261
  let { stdout: currentBranch } = execa.commandSync(
    'git rev-parse --abbrev-ref HEAD'
  );
  ...
  if ('master' === currentBranch) {
    ...
    console.log(
      chalk.red(
        `\n必须在 master 分支发布，请把 "dev" 相关代码合并到 master 再执行发布，发布完成再把 master 合并到 "dev" 分支\n`
      )
    );
    process.exit(1);
  }
...
const packages = fs
  ...
  .filter(
    p => !p.endsWith('.ts') && !p.startsWith('.') && !['server'].includes(p)
  );
...
async function main() {
  ...
  if (!skipGit) {
    step('\nPushing to GitHub...');
    await runIfNotDry('git', ['tag', `v${targetVersion}`, '-f']);
...
function updateDeps(pkg, depType, version, getNewPackageName) {
  ...
  Object.keys(deps).forEach(dep => {
    ...
    //   deps[dep] = newVersion;
    // }
    if (dep.startsWith('@ktlib/my-unplugin')) {
      console.log(
  ...
  try {
    await run(
      ...
      [
        ...
        '--registry',
        'https://ktsoft-npm.pkg.coding.net/frontpackage/ktlib/'
      ],
      {
        cwd: pkgRoot,
        stdio: 'pipe'
